...
func main() {
	...
	tracer, flush, err := tracing.Init(tracing.TracerInput{
		...
		ServiceName:   "exports-service-frontend",
		TracerName:    "exports-service-frontend-tracer",
	})
	if err != nil {
	...
	if err != nil {
		log.Critical(ctx, errors.New(fmt.Sprintf("can not initialize ExportsRepository %v", err)))
		return
	}
	...
	s, err := grpc.NewServer(grpc.ServerInput{
		...
		Registrator: func(srv ggrpc.ServiceRegistrar) {
			pb.RegisterExportsServiceFrontendServer(srv, frontend)
		},
	})
	if err != nil {
